...
def snmp_check(host: str, port: int = 161, timeout: int = 3, community: str = "public") -> dict:
    ...
    if community_len > 97:
        return {"error": f"community string too long ({community_len} bytes); max 97 for BER short-form encoding", "tool": "snmp_check"}
...
def check_monica(host: str, port: int = 80, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_monica"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_vikunja(host: str, port: int = 3456, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_vikunja"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_stirling_pdf(host: str, port: int = 8080, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_stirling_pdf"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_grocy(host: str, port: int = 9283, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_grocy"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_actual_budget(host: str, port: int = 5006, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_actual_budget"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_linkwarden(host: str, port: int = 3000, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_linkwarden"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_photoprism(host: str, port: int = 2342, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_photoprism"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_wallabag(host: str, port: int = 80, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_wallabag"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_tandoor(host: str, port: int = 8080, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_tandoor"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_outline(host: str, port: int = 3000, timeout: int = 5, https: bool = False) -> dict:
    ...
    host = host.strip()
    if not 1 <= port <= 65535:
        return {"error": f"port {port} out of range 1-65535", "tool": "check_outline"}
    scheme = "https" if https else "http"
    ctx = None
    if https:
        ctx = ssl.create_default_context()
...
def check_victoriametrics(host: str, port: int = 8428, timeout: int = 5, https: bool = False) -> dict:
    ...
    result: dict = {"host": host, "port": port, "reachable": healthy, "healthy": healthy, "status": body}
    try:
